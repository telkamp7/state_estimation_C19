...
In this project, the daily record of new hospital admissions with Covid-19 in Denmark grouped by region of residence and totals are used. This report is based on data from `r strftime(newly_admitted$Dato[1], format = "%F")` to `r strftime(newly_admitted$Dato[n], format = "%F")` is used. The head and tail of the processed data are listed in Table \@ref(tab:tbl).
...
glmmTMB.beta <- paste0(round(glmmTMB.summary$coefficients$cond[1],digits),
                       " (", round(glmmTMB.summary$coefficients$cond[2],digits),")")

KFAS.beta <- paste0(round(as.numeric(coef(object = m1.out.KFAS, last = TRUE)[1]), digits),
                    " (", round(sqrt(m1.out.KFAS$V[1,1,n]), digits),")")

parTbl <- data.frame(Parameter = "$\\beta$",
                     glmmTMB = glmmTMB.beta,
                     KFAS = KFAS.beta)

# parTbl <- data.frame(Parameter = "$\\beta$",
#                      glmmTMB = as.numeric(fixef(object = m1.glmmTMB)$cond),
#                      KFAS = as.numeric(coef(object = m1.out.KFAS, last = TRUE)[1]))
...
KFAS.reg.beta <- paste0(round(as.numeric(coef(object = m2.out.KFAS, last = TRUE)[1:6]), digits),
                    " (", round(sqrt(diag(m2.out.KFAS$V[,,n])),digits)[1:6],")")

parTblReg <- data.frame(Parameter = paste0("$\\beta_",1:6,"$"),
                        glmmTMB = glmmTMB.reg.beta,
                        KFAS = KFAS.reg.beta)

# parTblReg <- data.frame(Parameter = paste0("$\\beta_",1:6,"$"),
#                         glmmTMB = as.numeric(fixef(object = m2.glmmTMB)$cond),
#                         KFAS = as.numeric(coef(object = m2.out.KFAS, last = TRUE)[1:6]))
...
(ref:diagnosticRes) Diagnostic plots of the in-sample one-step prediction residuals. A) Time series of the residuals, B) auto-correlation function of the residuals, C) Histogram of the residuals, and D) Qunatile-Quantile plot of the residuals.
